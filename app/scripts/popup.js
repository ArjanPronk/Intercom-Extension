	...
	if (password.length > 3 && username.length > 3) {
		chrome.windows.getCurrent(function (currentWindow) {
			chrome.tabs.query({active: true, windowId: currentWindow.id, }, function(activeTabs) {
				chrome.tabs.executeScript(activeTabs[0].id, {file: 'lib/jq.js', allFrames: true});
...
chrome.runtime.onMessage.addListener(
	function(request, sender, sendResponse) {
      ...
      var searchIntercomUrl = 'https://app.intercom.io/apps/'+username+'/users?search=';
        ...
        jQuery.each(request.foundEmails, function (i, item) {
          jQuery('#resultList').append('<li><a href="https://app.intercom.io/a/apps/'+username+'/users/'+request.foundIds[i]+'/all-conversations">'+item+'</a></li>');
        });
